...
namespace MetadataComposite.Areas.Metadata
{
	public class MetadataAreaRegistration : AreaRegistration
	{
		...
		public override void RegisterArea(AreaRegistrationContext context)
		{
			...
			context.MapRoute(
				"AllAgentsWithFormat", "metadata/agents/index.{format}", new { controller = "Agents", action = "Index" });

			context.MapRoute("AllAgents", "metadata/agents", new { controller = "Agents", action = "Index" });

			context.MapRoute("Agent", "metadata/agents/{agentSystemName}", new { controller = "Agents", action = "ViewAgent" });

			context.MapRoute(
				"AgentWithFormat", "metadata/agents/{agentSystemName}.{format}", new { controller = "Agents", action = "ViewAgent" });

			context.MapRoute(
				"AgentPartsWithFormat", 
